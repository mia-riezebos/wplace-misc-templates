...
(() => {
  ...
  const PALETTE_BY_RGB = new Map(
    PALETTE.map((color) => [color.rgb.join(","), color]),
  );
  ...
  function editorColor(r, g, b) {
    const exact = PALETTE_BY_RGB.get(`${r},${g},${b}`);
    if (exact) return exact;
    return state.editorKind === "profile" ? { name: null, rgb: [r, g, b] } : null;
  }
  ...
  function validateTemplate(imageData) {
    for (let y = 0; y < imageData.height; y += 1) {
      for (let x = 0; x < imageData.width; x += 1) {
        ...
        if (state.editorKind === "alliance" && !PALETTE_BY_RGB.has(rgb)) {
